...
function Cart(props){
    function FindName(ids){
        let ans=[];
        DUMMY_MEALS.map(data=>{if(data.id==ids) ans=data});
        return ans;
    }
    const ct=useContext(items);
const CartItem=<ul className="cart-items ">{ct.id.map(data=>
    <li>
    <div className="prod">
...
return (
    ...
    <div className="fit">
        ...
        <button className="button--alt" onClick={props.change}>CLOSE</button>
